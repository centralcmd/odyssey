...
- **The tile grid goes `Dense`.** A Person card already ran up to nine tiles against the grid's comfortable ceiling of eight; the new facts take it to eleven. `Dense` is decided per record type, never mixed inside a card. A field with no value still renders no tile.
