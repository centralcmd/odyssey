...
**Where it surfaces.** `components/Avatar.jsx` (`fit`, `onError`, plain `<img>`) · `components/RecordCard.jsx` (the `image` mark) · `components/FileUpload.jsx` (`onFiles`) · `ui_kits/web/ContactAvatarDialog.jsx` (the crop dialog + `ContactAvatarLimits`) · `ui_kits/web/Contacts.jsx` (the list mark, the ⋯ menu items, the remove confirm). Specimens: `components/contact-avatar.html`, `preview/63-dialog-crop-image.html`, `preview/43-dialog-new-contact.html`.
