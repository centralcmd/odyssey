...
public class SystemSettingsBoundsTests
{
    ...
    /// 38 int keys before issue #437, 41 after.
    /// </summary>
    [Fact]
    public void The_int_key_census_is_forty_one()
    {
        ...
        // per-kind counts are right rather than merely consistent with each other.
        Assert.Equal(62, SystemSettingsRegistry.All.Count);
        Assert.Equal(4, SystemSettingsRegistry.All.OfType<BoolSetting>().Count());
        Assert.Equal(8, SystemSettingsRegistry.All.OfType<CapacitySetting>().Count());
        Assert.Equal(8, SystemSettingsRegistry.All.OfType<StringSetting>().Count());
