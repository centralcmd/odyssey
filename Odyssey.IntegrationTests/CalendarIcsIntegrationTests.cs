...
/// <summary>
/// Real-engine regression coverage for the PR #403 review fix, on the calendar-events aggregate export
...
/// per surface (a PR #403 test-review follow-up finding): <c>CalendarIcsService</c> chains a different
/// sort key (<c>StartDateTime</c>+<c>CalendarEventId</c>, vs. Contacts' single <c>ContactId</c>) and a
/// calendar-name lookup per chunk that Contacts doesn't have, so structural similarity to Contacts'
/// proven fix isn't itself proof this surface's fix actually runs against a real retrying engine.
/// <see cref="CalendarIcsService.ExportAggregateFilteredStreamingAsync"/> originally held one explicit
...
public class CalendarIcsIntegrationTests(MariaDbFixture fixture)
{
    [SkippableFact]
    public async Task Export_FilteredCalendarEvents_AcrossMultipleChunks_SucceedsAgainstRealEngineWithRetryOnFailure()
    {
        ...
        {
            var calendar = new Calendar { Name = $"Cal {suffix}", CreatedByUserId = "tester" };
            seeding.Calendars.Add(calendar);
            await seeding.SaveChangesAsync();

            var start = new DateTime(2026, 1, 1, 9, 0, 0, DateTimeKind.Utc);
            for (var i = 0; i < rowCount; i++)
            {
                seeding.CalendarEvents.Add(new CalendarEvent
                {
                    CalendarId = calendar.CalendarId,
                    Title = $"Chunked {suffix} {i}",
                    StartDateTime = start.AddMinutes(i),
                    EndDateTime = start.AddMinutes(i + 30),
        ...
        var service = new CalendarIcsService(
            context, NullLogger<CalendarIcsService>.Instance, new UnlimitedImportExportLimitsLookup(),
            new StubJournalLimitsLookup());

        using var output = new MemoryStream();
        var query = new CalendarEventsIcsExportQueryParams { Search = suffix }; // Search alone triggers the chunked, filtered path
        string? fileName = null;
        var reportedCount = 0;
        await service.ExportAggregateStreamingAsync(query, "tester", output, (name, count) =>
        {
            fileName = name;
            reportedCount = count;
        });
        ...
        Assert.Equal(rowCount, CountOccurrences(body, "BEGIN:VEVENT"));
    }
